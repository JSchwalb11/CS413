...
@    as -o Schwalb_Lab1.s.o Schwalb_Lab1.s.s
@    gcc -o Schwalb_Lab1.s Schwalb_Lab1.s.o
@    ./Schwalb_Lab1.s ;echo $?
@    gdb --args ./Schwalb_Lab1.s
...
loopArr1:
   ...
   add r4, r4, #4              @ post index r4 = r4 + 4
   add r2, r2, #1              @ increment counter
   cmp r2, #10                 @ compare counter with #11
   it lt                       @ if less than peusdocode
...
loopArr2:
   ldr r0, =loopArr2           @ load r0 with the memory address of loopArr2
   add r0, r0, #8              @ 4 instructions * 2 bytes = 8 byte offset
   mov lr, r0                  @ move calculated address into link register
...
@*******************
   ...
   pop {r4}                    @ pop into r4
   pop {r0}                    @ pop into r0
   mov lr, r0                  @ mov r0 into link register
...
@*******************   
   ...
   add r5, r5, #4              @ post index r5 = r5 + 4
   mul r1, r1, r0              @ multiply r1 * r0, store in r1
   mov r3, r1                  @ mov r1 into r3
   str r3, [r6]                @ store multiplication results stored in r3 into arr3
   add r6, r6, #4              @ post index r6 = r6 + 4
